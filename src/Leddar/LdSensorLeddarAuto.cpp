...
// *****************************************************************************
void
LeddarDevice::LdSensorLeddarAuto::Connect( void )
{
    LdDevice::Connect();
    ConnectDataServer();
}
...
// *****************************************************************************
void
LeddarDevice::LdSensorLeddarAuto::Disconnect( void )
{
    if( mProtocolData != nullptr )
    {
...
// *****************************************************************************
void
LeddarDevice::LdSensorLeddarAuto::ConnectDataServer( void )
{
    ...
    if( mIsTCPDataServer )
    {
        lConnectionInfoEthernetDataServer = new LeddarConnection::LdConnectionInfoEthernet(
            lConnectionInfoEthernet->GetIP(), mProperties->GetIntegerProperty( LdPropertyIds::ID_DATA_SERVER_PORT )->ValueT<uint32_t>(),
            "", lConnectionInfoEthernet->GetType(), LeddarConnection::LdConnectionInfoEthernet::PT_TCP );
    }
...
/// *****************************************************************************
void
LeddarDevice::LdSensorLeddarAuto::InitProperties( void )
{
    mProperties->AddProperty( new LdTextProperty( LdProperty::CAT_CONFIGURATION, LdProperty::F_EDITABLE | LdProperty::F_SAVE, LdPropertyIds::ID_DEVICE_NAME,
                              LtComLeddarTechPublic::LT_COMM_ID_DEVICE_NAME, LT_COMM_DEVICE_NAME_LENGTH, LeddarCore::LdTextProperty::TYPE_UTF8, "Device name" ) );

    mProperties->AddProperty( new LdTextProperty( LdProperty::CAT_CONSTANT, LdProperty::F_SAVE, LdPropertyIds::ID_PART_NUMBER,
    ...
    mProperties->AddProperty( new LdTextProperty( LdProperty::CAT_CONSTANT, LdProperty::F_SAVE, LdPropertyIds::ID_SERIAL_NUMBER,
                              LtComLeddarTechPublic::LT_COMM_ID_SERIAL_NUMBER, LT_COMM_SERIAL_NUMBER_LENGTH, LeddarCore::LdTextProperty::TYPE_ASCII, "Serial number" ) );
    mProperties->AddProperty( new LdTextProperty( LdProperty::CAT_CONSTANT, LdProperty::F_SAVE, LdPropertyIds::ID_FPGA_VERSION,
                              LtComLeddarTechPublic::LT_COMM_ID_FPGA_VERSION, LT_COMM_FPGA_VERSION_LENGTH, LeddarCore::LdTextProperty::TYPE_ASCII, "FPGA version" ) );
    mProperties->AddProperty( new LdTextProperty( LdProperty::CAT_CONSTANT, LdProperty::F_SAVE, LdPropertyIds::ID_GROUP_ID_NUMBER,
                              LtComLeddarTechPublic::LT_COMM_ID_GROUP_ID, LT_COMM_GROUP_ID_LENGTH, LeddarCore::LdTextProperty::TYPE_ASCII, "Group id" ) );
    mProperties->AddProperty( new LdIntegerProperty( LdProperty::CAT_CONSTANT, LdProperty::F_SAVE, LdPropertyIds::ID_RELEASE_TYPE,
                              LtComLeddarTechPublic::LT_COMM_ID_RELEASE_TYPE, 1, "Release type" ) );
    ...
    GetProperties()->AddProperty( new LdFloatProperty( LdProperty::CAT_CALIBRATION, LdProperty::F_EDITABLE | LdProperty::F_SAVE, LdPropertyIds::ID_TIMEBASE_DELAY,
                                  LtComLeddarTechPublic::LT_COMM_ID_TIMEBASE_DELAYS, 4, 0, 3, "Timebase delays" ) );
...
/// *****************************************************************************
void
LeddarDevice::LdSensorLeddarAuto::GetConstants( void )
{
...
void
LeddarDevice::LdSensorLeddarAuto::GetConfig( void )
{
...
void
LeddarDevice::LdSensorLeddarAuto::GetCalib( void )
{
...
bool
LeddarDevice::LdSensorLeddarAuto::GetData( void )
{
...
bool
LeddarDevice::LdSensorLeddarAuto::ProcessData( uint16_t aRequestCode )
{
...
////////////////////////////////////////////////////////////////////////////////////////////////////
bool
LeddarDevice::LdSensorLeddarAuto::RequestData( uint32_t &aMask )
{
...
// *****************************************************************************
bool
LeddarDevice::LdSensorLeddarAuto::ProcessEchoes( void )
{
        ...
        if( !lFlush || mProtocolData->GetElementId() == LtComLeddarTechPublic::LT_COMM_ID_STATUS )
        {
            switch( mProtocolData->GetElementId() )
            {
                case LtComLeddarTechPublic::LT_COMM_ID_TIMESTAMP:
                    ...
                    if( lTimestamp + std::numeric_limits<int16_t>::max() < mEchoes.GetTimestamp( LeddarConnection::B_SET ) ) //Timestamp loop
                    {
                        ...
                        mEchoes.SetTimestamp( lTimestamp );
                             ...
                             ( lTimestamp <= mEchoes.GetTimestamp( LeddarConnection::B_GET ) && lTimestamp + std::numeric_limits<int16_t>::max() > mEchoes.GetTimestamp( LeddarConnection::B_GET ) ) )
                    {
                ...
                case LtComLeddarTechPublic::LT_COMM_ID_AUTO_TIMESTAMP64:
                {
                    ...
                    mProtocolData->PushElementDataToBuffer( &lTimestamp64,
                                                            mProtocolData->GetElementCount(),
                                                            sizeof( uint64_t ),
                                                            sizeof( uint64_t ) );
                    mEchoes.SetTimestamp64( lTimestamp64 );
                }
                break;
                ...
                {
                    uint64_t lFrameId = 0;
                    mProtocolData->PushElementDataToBuffer( &lFrameId,
                                                            mProtocolData->GetElementCount(),
                                                            sizeof( uint64_t ),
                                                            sizeof( uint64_t ) );
                    mEchoes.SetFrameId( lFrameId );
                }
                break;

                case LtComLeddarTechPublic::LT_COMM_ID_AUTO_NUMBER_DATA_SENT:
                    mProtocolData->PushElementDataToBuffer( lStartIndexAndCount,
                                                            mProtocolData->GetElementCount(),
                                                            sizeof( uint32_t ),
                                                            sizeof( uint32_t ) );
                ...
                case LtComLeddarTechPublic::LT_COMM_ID_AUTO_ECHOES_AMPLITUDE:
                    mProtocolData->PushElementDataToBuffer( &lEchoes[lStartIndexAndCount[0]].mAmplitude,
                                                            mProtocolData->GetElementCount(),
                ...
                case LtComLeddarTechPublic::LT_COMM_ID_AUTO_ECHOES_DISTANCE:
                    mProtocolData->PushElementDataToBuffer( &lEchoes[lStartIndexAndCount[0]].mDistance,
                                                            mProtocolData->GetElementCount(),
                ...
                case LtComLeddarTechPublic::LT_COMM_ID_AUTO_ECHOES_CHANNEL_INDEX:
                    mProtocolData->PushElementDataToBuffer( &lEchoes[lStartIndexAndCount[0]].mChannelIndex,
                                                            mProtocolData->GetElementCount(),
                ...
                case LtComLeddarTechPublic::LT_COMM_ID_AUTO_ECHOES_VALID:
                    mProtocolData->PushElementDataToBuffer( &lEchoes[lStartIndexAndCount[0]].mFlag,
                                                            mProtocolData->GetElementCount(),
                ...
                case LtComLeddarTechPublic::LT_COMM_ID_AUTO_ECHOES_TIMESTAMP_UTC :
                    mProtocolData->PushElementDataToBuffer( &lEchoes[lStartIndexAndCount[0]].mTimestamp,
                                                            mProtocolData->GetElementCount(),
                                                            sizeof( ( ( LeddarConnection::LdEcho * )0 )->mTimestamp ),
                                                            sizeof( lEchoes[ 0 ] ) );
                    break;

                case LtComLeddarTechPublic::LT_COMM_ID_STATUS:

                    mProtocolData->PushElementDataToBuffer( &lDataReceivedStatus,
                                                            mProtocolData->GetElementCount(),
                                                            sizeof( lDataReceivedStatus ),
                                                            sizeof( lDataReceivedStatus ) );

                    if( lDataReceivedStatus )
                        mEchoes.SetScanDirection( lDataReceivedStatus > 1 ? 1 : 0 ); // currently only used for eagle 312lines
...
// *****************************************************************************
bool
LeddarDevice::LdSensorLeddarAuto::ProcessStates( void )
{
    SetDataReceived( true );
...
// *****************************************************************************
void
LeddarDevice::LdSensorLeddarAuto::SetDataMask( uint32_t aDataMask )
{
    mDataMask = aDataMask;
    ...
    {
        throw LeddarException::LtComException( "Set data mask error, request code: " + LeddarUtils::LtStringUtils::IntToString( LtComLeddarTechPublic::LT_COMM_CFGSRV_REQUEST_SET )
                                               + " wrong answer code: " + LeddarUtils::LtStringUtils::IntToString( mProtocolConfig->GetAnswerCode() ), LeddarException::ERROR_COM_WRITE );
    }
}
...
void
LeddarDevice::LdSensorLeddarAuto::SetConfig( void )
{
...
void
LeddarDevice::LdSensorLeddarAuto::WriteConfig( void )
{
    SendCommand( LtComLeddarTechPublic::LT_COMM_CFGSRV_REQUEST_WRITE_CONFIG );
}
...
void
LeddarDevice::LdSensorLeddarAuto::RestoreConfig( void )
{
...
// *****************************************************************************
void
LeddarDevice::LdSensorLeddarAuto::Reset( LeddarDefines::eResetType aType, LeddarDefines::eResetOptions aOptions, uint32_t )
{
    ...
    {
        uint8_t lSoftwareType = ( aOptions == LeddarDefines::RO_FACTORY ? LtComLeddarTechPublic::LT_COMM_SOFTWARE_TYPE_FACTORY : LtComLeddarTechPublic::LT_COMM_SOFTWARE_TYPE_MAIN );
...
// *****************************************************************************
void
LeddarDevice::LdSensorLeddarAuto::RequestProperties( LeddarCore::LdPropertiesContainer *aProperties, std::vector<uint16_t> aDeviceIds )
{
    mPingEnabled = false;
    ...
    mProtocolConfig->AddElement( LtComLeddarTechPublic::LT_COMM_ID_ELEMENT_LIST, static_cast<uint16_t>( aDeviceIds.size() ), sizeof( uint16_t ), &aDeviceIds[ 0 ], sizeof( uint16_t ) );
    mProtocolConfig->SendRequest();
...
// *****************************************************************************
void
LeddarDevice::LdSensorLeddarAuto::SetProperties( LeddarCore::LdPropertiesContainer *aProperties, std::vector<uint16_t> aDeviceIds, unsigned int aRetryNbr )
{
    mPingEnabled = false;
    LeddarUtils::LtScope<bool> lPingEnabler( &mPingEnabled, true );
    ...
    for( size_t i = 0; i < aDeviceIds.size(); ++i )
    {
        ...
        {
            mProtocolConfig->StartRequest( LtComLeddarTechPublic::LT_COMM_CFGSRV_REQUEST_SET );
            mProtocolConfig->AddElement( aDeviceIds[ i ], static_cast<uint16_t>( lProperty->Count() ), lProperty->UnitSize(),
                                         lProperty->CStorage(), static_cast<uint32_t>( lProperty->Stride() ) );
            mProtocolConfig->SendRequest();
            ...
            do
            {
                try
                {
                    ...
                    if( mProtocolConfig->GetAnswerCode() != LtComLeddarTechPublic::LT_COMM_ANSWER_OK )
                    {
                        throw LeddarException::LtComException( "Set properties error, request code: " + LeddarUtils::LtStringUtils::IntToString( LtComLeddarTechPublic::LT_COMM_CFGSRV_REQUEST_SET )
                                                               + " wrong answer code: " + LeddarUtils::LtStringUtils::IntToString( mProtocolConfig->GetAnswerCode() ), LeddarException::ERROR_COM_WRITE );
                    }
                }
                catch( LeddarException::LtComException &e )
...
void
LeddarDevice::LdSensorLeddarAuto::GetCategoryPropertiesFromDevice( LdProperty::eCategories aCategory, uint16_t aRequestCode )
{
    mPingEnabled = false;
    LeddarUtils::LtScope<bool> lPingEnabler( &mPingEnabled, true );
...
void
LeddarDevice::LdSensorLeddarAuto::SetCategoryPropertiesOnDevice( LdProperty::eCategories aCategory, uint16_t aRequestCode )
{
    mPingEnabled = false;
    LeddarUtils::LtScope<bool> lPingEnabler( &mPingEnabled, true );
    mProtocolConfig->StartRequest( aRequestCode );
...
void
LeddarDevice::LdSensorLeddarAuto::SendCommand( uint16_t aRequestCode, unsigned int aRetryNbr )
{
    mPingEnabled = false;
    LeddarUtils::LtScope<bool> lPingEnabler( &mPingEnabled, true );
    mProtocolConfig->StartRequest( aRequestCode );
        ...
        try
        {
            ...
            if( mProtocolConfig->GetAnswerCode() != LtComLeddarTechPublic::LT_COMM_ANSWER_OK )
            {
                throw LeddarException::LtComException( "Send command, request code: " + LeddarUtils::LtStringUtils::IntToString( aRequestCode )
                                                       + " wrong answer code: " + LeddarUtils::LtStringUtils::IntToString( mProtocolConfig->GetAnswerCode() ), LeddarException::ERROR_COM_UNKNOWN );
            }
        }
        catch( LeddarException::LtComException &e )
        {
...
// *****************************************************************************
void
LeddarDevice::LdSensorLeddarAuto::SendPing( void )
{
    if( mPingEnabled )
...
LeddarDevice::LdSensorLeddarAuto::SendLicense( const uint8_t *aLicense, bool aVolatile = false )
{
...
// *****************************************************************************
LeddarDefines::sLicense
LeddarDevice::LdSensorLeddarAuto::SendLicense( const std::string &aLicense, bool aVolatile )
{
...
LeddarDevice::LdSensorLeddarAuto::GetLicenses( void )
{
...
void
LeddarDevice::LdSensorLeddarAuto::RemoveLicense( const std::string &aLicense )
{
    LdBufferProperty *lLicenseProp = GetProperties()->GetBufferProperty( LdPropertyIds::ID_LICENSE );
...
////////////////////////////////////////////////////////////////////////////////////////////////////
/// \fn void LeddarDevice::LdSensorLeddarAuto::UpdateFirmware( eFirmwareType aFirmwareType, const LdFirmwareData& aFirmwareData, LeddarCore::LdIntegerProperty* aProcessPercentage, LeddarCore::LdBoolProperty* aCancel )
///
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void
LeddarDevice::LdSensorLeddarAuto::UpdateFirmware( eFirmwareType aFirmwareType, const LdFirmwareData &aFirmwareData, LeddarCore::LdIntegerProperty *aProcessPercentage,
        LeddarCore::LdBoolProperty *aCancel )
{
    ...
    switch( aFirmwareType )
    {
        ...
        case FT_OS: lFirmwareType = LtComLeddarTechPublic::LT_COMM_SOFTWARE_TYPE_OS; break;

        default: throw LeddarException::LtException( "Invalid firmware type: " + LeddarUtils::LtStringUtils::IntToString( aFirmwareType ) );
    }
    ...
    mProtocolConfig->AddElement( LtComLeddarTechPublic::LT_COMM_ID_OPEN_UPDATE_SESSION, 1, sizeof( lOpenCloseSessionFlag ), &lOpenCloseSessionFlag, sizeof( lOpenCloseSessionFlag ) );
    mProtocolConfig->SendRequest();
    ...
    if( mProtocolConfig->GetAnswerCode() != LtComLeddarTechPublic::LT_COMM_ANSWER_OK )
    {
        throw LeddarException::LtComException( "Update firmware error, request code: " + LeddarUtils::LtStringUtils::IntToString( LtComLeddarTechPublic::LT_COMM_CFGSRV_REQUEST_UPDATE )
                                               + " wrong answer code: " + LeddarUtils::LtStringUtils::IntToString( mProtocolConfig->GetAnswerCode() ), LeddarException::ERROR_COM_WRITE );
    }
    ...
    mProtocolConfig->StartRequest( LtComLeddarTechPublic::LT_COMM_CFGSRV_REQUEST_UPDATE );
    mProtocolConfig->AddElement( LtComLeddarTechPublic::LT_COMM_ID_PROCESSOR, 1, sizeof( lFirmwareType ), &lFirmwareType, sizeof( lFirmwareType ) );
    mProtocolConfig->AddElement( LtComLeddarTechPublic::LT_COMM_ID_CLOSE_UPDATE_SESSION, 1, sizeof( lOpenCloseSessionFlag ), &lOpenCloseSessionFlag, sizeof( lOpenCloseSessionFlag ) );
    mProtocolConfig->SendRequest();
    ...
    do
    {
        try
        {
            ...
            {
                throw LeddarException::LtComException( "Update firmware error, request code: " + LeddarUtils::LtStringUtils::IntToString( LtComLeddarTechPublic::LT_COMM_CFGSRV_REQUEST_UPDATE )
                                                       + " wrong answer code: " + LeddarUtils::LtStringUtils::IntToString( mProtocolConfig->GetAnswerCode() ), LeddarException::ERROR_COM_WRITE, true );
